...
#### Thursday (10/30)
- tested out iqtree command to see if it works before running pipe2slurm
  - it couldn't find the index file (only gave it the vcf to work with)
  - tried a little bit to troubleshoot, but gave up and sent a slack message to Andrius
  - waiting to hear back response, so I'm going to work on finding location data now
    - ok I'm adding to the metadata and I am now discovering that some of these samples are not P. destructans but a very closely related species. Which would probably mess up the population structure analysis. probably.